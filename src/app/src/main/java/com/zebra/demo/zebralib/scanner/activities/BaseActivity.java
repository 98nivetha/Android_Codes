...
import static com.zebra.demo.zebralib.scanner.helpers.Constants.logAsMessage;
...
public class BaseActivity extends AppCompatActivity implements ScannerAppEngine, IDcsSdkApiDelegate {
    ...
    ActiveDeviceActivity mActiveDeviceActivity;
    public ArrayList<Barcode> barcodeQueue = new ArrayList<Barcode>();

    // The Handler that gets information back from the BluetoothChatService
    protected final Handler mHandler = initializeHandler();
    protected static  boolean waitingForFWReboot = false;

    public void setParentActivity(ActiveDeviceActivity activity)
    {
        mActiveDeviceActivity = activity;
    }
    @Override
...
//        }
    ...
    protected void initScanner()
    {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                Application.sdkHandler.dcssdkSetDelegate(mActiveDeviceActivity);
    ...
    protected void setWaitingForFWReboot(boolean _waitingForFWReboot){
    ...
    public DCSSDKDefs.DCSSDK_RESULT connect(int scannerId) {
     ...
     //       }
            Application.sdkHandler.dcssdkEnableAutomaticSessionReestablishment(false, scannerId);
            Application.sdkHandler.dcssdkEnableBluetoothScannersDiscovery(false);
    ...
    public void dcssdkEventBarcode(byte[] barcodeData, int barcodeType, int fromScannerID) {
        runOnUiThread(() -> {
            ...
            if( mActiveDeviceActivity!= null)
                mActiveDeviceActivity.scannerBarcodeEvent(barcodeData,barcodeType,fromScannerID);
        });
        ...
        public void handleMessage(Message msg) {
            switch(msg.what){
                ...
                case Constants.BARCODE_RECEIVED:
                    ...
                    View mView = findViewById(R.id.btnScanTrigger);
                    if(mView != null ) {
                        mView.setOnClickListener(new View.OnClickListener() {
                            @Override
                            public void onClick(View v) {
                                if (mActiveDeviceActivity != null)
                                    mActiveDeviceActivity.scanTrigger(mView);
                            }
                        });
                    }
                    Barcode barcode=(Barcode)msg.obj;
                    ...
                    if(!Application.isFirmwareUpdateInProgress) {
                        if (Application.currentScannerId != Application.SCANNER_ID_NONE) {
                            Intent intent = new Intent("ActiveDeviceActivity");
                            intent.setComponent(new ComponentName(getPackageName(), ActiveDeviceActivity.class.getName()));
                ...
                case Constants.SESSION_ESTABLISHED:
                    ...
                    setAutoReconnectOption(activeScanner.getScannerID(),false);
                    ...
                    if (mOfflineScannerInfoList != null) {
                        for (DCSScannerInfo off_info : mOfflineScannerInfoList) {
                            if ((off_info != null ) && (off_info.getScannerID() == activeScanner.getScannerID())) {
                ...
                /* update dev list */
                    found = false;
                    for (DCSScannerInfo ex_info : mScannerInfoList) {
                        if ( (ex_info != null) && (ex_info.getScannerID() == scannerID)) {
