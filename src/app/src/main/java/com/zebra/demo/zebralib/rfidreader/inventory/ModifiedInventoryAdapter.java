...
public class ModifiedInventoryAdapter extends ArrayAdapter<InventoryListItem> {
    ...
    public synchronized View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            if( listItem.getMemoryBank() == null ||  listItem.getMemoryBank().contains("none"))
                ...
                holder = new InventoryViewHolder(textViewWrap, memoryBankData, count, rssi, memoryBank, text, pcView, rssiView, phaseView, channelView, csvTagDetailsView);
        ...
        if (listItem.getMemoryBankData() == null || (listItem.getMemoryBankData() != null && listItem.getMemoryBankData().isEmpty())) {
            ...
            if (listItem.getMemoryBank() != "none")
                holder.getMemoryBank().setText("MEMORY_BANK_EPC_MEMORY");
            holder.getMemoryBankData().setText(listItem.getMemoryBankData());


            if((Application.memoryBankId == 5) && (listItem.getMemoryBankData() != null) ) {
                if(RFIDController.asciiMode == false) {
                    int epcData = Integer.parseInt(listItem.getMemoryBankData(), 16);
                    if ((epcData & 0x8000) > 0) {
                        holder.getMemoryBankData().setTextColor(Color.RED);
                        holder.getTextView().setTextColor(Color.RED);
                    } else {
                        holder.getMemoryBankData().setTextColor(Color.BLACK);
                        holder.getTextView().setTextColor(Color.BLACK);
